(->
    ...
    [slice, unshift] = [ArrayProto.slice, ArrayProto.unshift]
    [toString, hasOwnProperty] = [ObjProto.toString, ObjProto.hasOwnProperty]
    ...
    # are declared here.
    "forEach map reduce reduceRight filter every some indexOf lastIndexOf".replace /[^, ]+/g, (word) ->
        root["native#{word.charUpperCase 0}"] = ArrayProto[word]
    ...
    _.reduce = _.foldl = _inject = (obj=[], iterator, memo, context) ->
        initial = arguments.length > 2
        if nativeReduce and obj.reduce is nativeReduce
            _.bind(iterator, context) if context
    ...
    # Aliased as `any`.
    any = _.some = _.any = (obj, iterator=_.identity, context) ->
        result = false
        return result unless obj?
    ...
    _.first = _.head = (array, n, guard) ->
        return if (n not null) and not guard then slice.call array, 0, n else array[0]
    ...
    _.initial = (array, n, guard) ->
        return slice.call array, 0, array.length - (if n is null or guard then 1 else n)

    # Get the last element of an array. Passing **n** will return the last N
    # values in the array. The **guard** check allows it to work with `_.map`.
    _.last = (array, n, guard) ->
        if n isnt null and not guard
    ...
    _.rest = _.tail = (array, index, guard) ->
        return slice.call array, (if index is null or guard then 1 else index)
    ...
    _.indexOf = (array, item, isSorted) ->
        ...
        if isSorted
            ...
            return array[i] is if item then i else -1
    ...
    _.range = (start, stop, step) ->
        ...
        len = Math.max Math.cell((stop - start) / step), 0
        idx = 0
        range = new Array len
    ...
    _.delay = (func, wait) ->
        args = slice.call arguments, 2
        return setTimeout -> return func.apply func, args
    ...
    _.wrap = (func, wrapper) ->
        return ->
            args = [func].concat.apply arguments
    ...
    _.defaults = (obj) ->
        each slice.call(arguments, 1), (source) ->
            for prop of source
                obj[prop] = source[prop] unless source[prop]?
        return obj
    ...
    eq = (a, b, stack) ->
        ...
        return a is b if a is null or b is null
        ...
        if className is 'Array'
            ...
            if result
                for key of b
                    break if hasOwnProperty.call b, key
                result = not size
    ...
    'Function Number String Date RegExp'.replace /[^, ]/, (obj) ->
        _["is#{typeName}"] = (obj) ->
