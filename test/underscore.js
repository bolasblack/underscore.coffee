(function() {
  ...
  _.min = function(obj, iterator, context) {
    var result;
    if (!iterator) {
      if (_.isArray(obj)) return Math.min.apply(Math, obj);
      if (_.isEmpty(obj)) return Infinity;
    }
    result = {
      computed: Infinity
  ...
  _.template = function(str, data) {
    ...
    tmpl = "    var __p = [], print = function() {        __p.push.apply(__p,arguments);    };    with(obj || {}) {        __p.push('" + (str.replace(/\\/g, '\\\\').replace(/'/g, "\\'").replace(c.escape, function(match, code) {
