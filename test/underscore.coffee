...
_.max = (obj, iterator, context) ->
    unless iterator
        return Math.max.apply Math, obj if _.isArray(obj) and obj[0] is +obj[0]
...
_.min = (obj, iterator, context) ->
    unless iterator
        return Math.min.apply Math, obj if _.isArray obj and obj[0] is +obj[0]
        return Infinity if _.isEmpty obj
    result = computed: Infinity
    each obj, (value, index, list) ->
        computed = if iterator then iterator.call context, value, index, list else value
...
_.memoize = (func, hasher) ->
    ...
    return ->
        ...
        return if _.has memo, key then memo[key] else memo[key] = func.apply @, arguments
...
_.debounce = (func, wait, immediate) ->
    ...
    return ->
        ...
        later = =>
            timeout = null
            func.apply @, args if !immediate
        func.apply context, args if immediate and !timeout
    ...
    if className is 'Array'
        ...
        for key of a
            if _.has a, key
                ...
                break unless result = _.has(b, key) and eq a[key], b[key], stack
